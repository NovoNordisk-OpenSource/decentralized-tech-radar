...
func Print() string {
	// fmt.Println(config())
	// fmt.Println(requireConfig())
	// fmt.Println(utilFactory())
	// fmt.Println(utilautoComplete())
	// fmt.Println(utilHtmlUtil())
	// fmt.Println(utilInputSanitizer())
	// fmt.Println(utilMathUtils())
	// fmt.Println(utilQueryParamProcessor())
	// fmt.Println(utilRingCalculator())
	// fmt.Println(utilStringUtil())
	// fmt.Println(utilUrlUtils())
	// fmt.Println(modelsBlip())
	// fmt.Println(modelsQuadrant())
	// fmt.Println(modelsRadar())
	// fmt.Println(modelsRing())
	// fmt.Println(graphingComponentsBanner())
	// fmt.Println(graphingComponentsButtons())
	// fmt.Println(graphingComponentsQuadrants())
	// fmt.Println(graphingComponentsQuadrantSubnav())
	// fmt.Println(graphingComponentsQuadrantTables())
	// fmt.Println(graphingComponentsSearch())
	// fmt.Println(graphingBlips())
	// fmt.Println(graphingConfig())
	// fmt.Println(graphingPdfPage())
	// fmt.Println(graphingRadar())

	bigscript := ""
	bigscript += config()
	bigscript += requireConfig()
	bigscript += utilFactory()
	bigscript += utilautoComplete()
	bigscript += utilHtmlUtil()
	bigscript += utilInputSanitizer()
	bigscript += utilMathUtils()
	bigscript += utilQueryParamProcessor()
	bigscript += utilRingCalculator()
	bigscript += utilStringUtil()
	bigscript += utilUrlUtils()
	bigscript += modelsBlip()
	bigscript += modelsQuadrant()
	bigscript += modelsRadar()
	bigscript += modelsRing()
	bigscript += graphingBlips()
	bigscript += graphingConfig()
	bigscript += graphingPdfPage()
	bigscript += graphingRadar()
	bigscript += graphingComponentsBanner()
	bigscript += graphingComponentsButtons()
	bigscript += graphingComponentsQuadrants()
	bigscript += graphingComponentsQuadrantSubnav()
	bigscript += graphingComponentsQuadrantTables()
	bigscript += graphingComponentsSearch()
...
func requireConfig() string {
	...
	require.config({
		paths: {
		  ...
		  'jquery-autocomplete': 'https://cdn.jsdelivr.net/npm/jqueryui@1.11.1/jquery-ui.min'   
		},
		shim: {
		  'd3tip': {
...
func utilInputSanitizer() string {
	script := `define([
		'sanitize-html',
		'lodash'],
...
func utilQueryParamProcessor() string {
	script := `
	define(function() {
...
func utilRingCalculator() string {
	script := `
	define(function() {
...
func utilStringUtil() string {
	...
	define(function() {
		...
		return {
		  getRingIdString: getRingIdString,
		  replaceSpaceWithHyphens: replaceSpaceWithHyphens,
...
func utilUrlUtils() string {
	script := `
	define(['../util/queryParamProcessor.js'], function(QueryParams) {
		...
		return {
		  constructSheetUrl: constructSheetUrl,
		  getDocumentOrSheetId: getDocumentOrSheetId,
...
func graphingBlips() string {
	...
	define([
	  ...
	  return {
		calculateRadarBlipCoordinates: calculateRadarBlipCoordinates,
		plotRadarBlips: plotRadarBlips,
		getRingRadius: getRingRadius,
		groupBlipsBaseCoords: groupBlipsBaseCoords,
		transposeQuadrantCoords: transposeQuadrantCoords,
		getGroupBlipTooltipText: getGroupBlipTooltipText,
		blipAssistiveText: blipAssistiveText,
		createGroupBlip: createGroupBlip,
		thereIsCollision: thereIsCollision
	  }
	});`
	return script
}

func graphingConfig() string {
	script := `
	define(function() {
...
func graphingPdfPage() string {
	script := `define(['d3'], function(d3) {
...
func graphingRadar() string {
	script := `define([
		...
		'./components/quadrantSubnav.js',
		'./components/search.js',
		'./components/buttons.js',
		'./components/quadrants.js',
		'./components/quadrantTables.js',
		'./pdfPage.js',
		'../util/urlUtils.js',
		'../util/mathUtils.js',
		'd3',
		...
		const { graphConfig, getGraphSize } = config;
		...
		const { renderQuadrantTables } = quadrantTables;
		const { addQuadrantNameInPdfView, addRadarLinkInPdfView } = pdfPage;
		const constructSheetUrl = urlUtils;
		const { toRadian } = mathUtils;
...
func graphingComponentsBanner() string {
	script := `define(['d3', '../../config.js', '../pdfPage.js'], function(d3, configFuntion, pdfPage) {
		const featureToggles = configFuntion().featureToggles;
...
func graphingComponentsButtons() string {
	script := `define(function() {
...
func graphingComponentsQuadrants() string {
	script := `define([
		'd3',
		'../../util/htmlUtil.js',
		'../../util/mathUtils.js',
		...
		return {
		  selectRadarQuadrant: selectRadarQuadrant,
		  renderRadarQuadrants: renderRadarQuadrants,
		  renderRadarLegends: renderRadarLegends,
		  renderMobileView: renderMobileView,
		  mouseoverQuadrant: mouseoverQuadrant,
		  mouseoutQuadrant: mouseoutQuadrant,
		  stickQuadrantOnScroll: stickQuadrantOnScroll,
		  removeScrollListener: removeScrollListener,
		  wrapQuadrantNameInMultiLine: wrapQuadrantNameInMultiLine,
		}
...
func graphingComponentsQuadrantSubnav() string {
	script := `define([
		'd3',
		'./quadrants.js',
		'../../util/stringUtil.js',
		...
		return renderQuadrantSubnav
	  });
	  `
	return script
}

func graphingComponentsQuadrantTables() string {
	script := `define([
		'd3',
		'../config.js',
		'./quadrants.js',
		...
		return {
		  renderQuadrantTables: renderQuadrantTables,
		  renderBlipDescription: renderBlipDescription,
		};
	  });
	  `
	return script
...
func graphingComponentsSearch() string {
	script := `define([
		'd3',
		'../../util/autoComplete',
...
func scriptAll() string {
	...
	define([
		...
		'../config.js',
		'../graphing/config.js'
	  ], function(d3, _, $, autocomplete, Radar, Quadrant, Ring, Blip, GraphingRadar, configFuntion, config) {
